mod native {
    use libc::c_char;
    use std::ffi::CString;
    use vk_sys::{Instance, SurfaceKHR};

    #[link(kind = "dylib", name = "gtkwin")]
    extern "C" {

        fn gtk_get_vulkan_surface(instance: Instance) -> SurfaceKHR;

        fn gtk_set_title(title: *const c_char);

        fn gtk_set_size(width: i32, height: i32);

        fn gtk_is_close_button_pressed() -> i32;
    ...
    pub fn window_should_close() -> bool {
        unsafe { gtk_is_close_button_pressed() != 0 }
    }
    ...
    pub fn create_window_and_show(width: i32, height: i32, title: &str) {
        ...
        unsafe {
            gtk_create_window_and_show(width, height, cstr.as_ptr());
        }
    }
...
fn main() {
    window_init();
    create_window_and_show(640, 480, "Haha, this window is \"safe\"");
